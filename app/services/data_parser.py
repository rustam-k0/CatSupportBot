...
def _get_date_patterns() -> list:
    ...
    УЛУЧШЕНИЕ: Добавлены шаблоны для английских дат с разделителями (•, - и т.д.)
    """
    return [
        # TIER 0: Самые надежные шаблоны. Ищем дату рядом с явными ключевыми словами операции.
        {
            ...
            'regex': r'(?:Операция\s+совершена|Дата\s+операции)[:\s]*(\d{1,2})\s+(января|февраля|марта|апреля|мая|июня|июля|августа|сентября|октября|ноября|декабря)\s+(\d{4})(?:\s+в\s+\d{1,2}:\d{2})?',
            'type': 'textual_ru',
            'flags': re.IGNORECASE
        },
        {
        ...
        # TIER 2: Английские даты с разделителями (новый шаблон для случаев типа "26 February • 16:08")
        {
            'tier': 2,
...
def _get_amount_patterns() -> list:
    ...
    AMOUNT_REGEX = r'(\d{1,3}(?:\s?\d{3})*(?:[,.]\d{1,2})?)'
    CURRENCY_REGEX = r'(?:Р|₽|руб\.?|RUB|P)'

    return [
        {'tier': 0, 'desc': 'Сумма с явным знаком "+" и символом валюты', 'regex': fr'\+\s*{AMOUNT_REGEX}\s*{CURRENCY_REGEX}', 'flags': re.IGNORECASE},
        {'tier': 1, 'desc': 'Ключевое слово "Сумма/Итого/Всего" и число', 'regex': fr'(?:Сумма|Итого|Всего|К\sоплате|Пополнение|Перевод)\s*[:\s.]*\s*{AMOUNT_REGEX}', 'flags': re.IGNORECASE},
        {'tier': 2, 'desc': 'Ключевое слово на отдельной строке ВЫШЕ числа', 'regex': fr'(?:Сумма|Итого|Всего|Операция|Сумма\sв\sвалюте\sоперации)\s*\n+\s*{AMOUNT_REGEX}\s*{CURRENCY_REGEX}?', 'flags': re.IGNORECASE},
...
def _get_procedure_patterns() -> list:
    ...
    return [{'tier': 1, 'desc': 'Блок текста между "Наименование" и "Итого"', 'regex': r'(?:Наименование|Описание\sуслуг)\s*\n(.*?)(?=\n\s*(?:Итого|Всего|Сумма))', 'flags': re.DOTALL | re.IGNORECASE}]
...
def parse_date(text: str) -> str | None:
    """Ищет ДАТУ и нормализует к виду ДД.ММ.ГГГГ. УЛУЧШЕНИЕ: Добавлена поддержка английских дат с разделителями."""
    ...
    # Собираем все найденные даты с их приоритетами
    found_dates = []
        ...
        for match in matches:
            try:
                ...
                if p.get('type') == 'textual_ru':
                    ...
                    # Уточняем формат года: гг или гггг
    ...
    # Выбираем дату с наивысшим приоритетом (наименьший tier)
    if found_dates:
